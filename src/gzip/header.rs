...
pub(crate) fn parse_gzip_member(
    ...
    // In a partial extraction scenario, we only validate the CRC32 if NO back-references
    // were encountered. If back-references were present, the extracted literals are only a subset
    // of the original decompressed data, making full CRC32 validation impossible without full decompression.
    if !had_matches && expected_crc32 != actual_crc32 {
        return Err(ZiftError::InvalidData {
            offset: reader.byte_pos,
            reason: format!("CRC32 mismatch for literal stream: expected {expected_crc32:08x}, got {actual_crc32:08x}"),
        });
    }
...
fn parse_gzip_header(reader: &mut BitReader<'_>) -> Result<(), ZiftError> {
    if reader.bit_pos != 0 {
        return Err(ZiftError::InvalidData {
            offset: reader.byte_pos,
            reason: "gzip header must start on byte boundary".to_string(),
        });
    }
    ...
    if reader.peek_u8()? != GZIP_MAGIC[0] {
        return Err(ZiftError::InvalidData {
            offset: reader.byte_pos,
            reason: "invalid gzip magic number".to_string(),
        });
    }
    ...
    if compression_method != 8 {
        return Err(ZiftError::InvalidData {
            ...
            reason: "unsupported gzip compression method (expected DEFLATE)".to_string(),
        });
    }
    ...
    if flags & 0xE0 != 0 {
        return Err(ZiftError::InvalidData {
            offset: reader.byte_pos.saturating_sub(1),
            reason: "invalid gzip flag bits".to_string(),
        });
    }
